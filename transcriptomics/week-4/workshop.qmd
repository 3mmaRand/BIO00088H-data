...
from the **`GenomicRanges`** package which is used by
**`DESeq2`**.
...
```
Now go to [Differential Expression Analysis](#frog-development-2).

## 🎄 *Arabidopisis*

We need to import the wildtype data that were filtered to remove genes with
3 or 4 zeros and those where the total counts was less than 20.
...
#| echo: false
#---CODING ANSWER---
wild_filtered <- read_csv("data-processed/wild_filtered.csv")
```

Now go to [Differential Expression Analysis](#arabidopisis-2).
...
Now go to [Differential Expression Analysis](#leishmania-2).
...
Now go to [Differential Expression Analysis](#stem-cells-2).
...
columns give the "treatments". In this case, the treatments stage (with
three levels) and treatment (with two levels).
...
treatment and sibling_rep.
We are interested in the difference between the treatments but we 
include `sibling_rep` to account for the fact that the data are paired. 
...
counts we again used the `counts()` function but this time we use 
the `normalized=TRUE` argument.
...
adjusted  *p*-value for the comparison between the control and the 
FGF-treatment for each gene.
...
It is useful to have the normalised counts and the statistical 
results in one dataframe. 
...
Now go to [Add gene information](#frog-development-3).
...
Now go to [Add gene information](#arabidopisis-3).
...
The genes expressed in only one treatment group are those with zeros in
all replicates in one group and non-zero values in all 
replicates in the other group. 
...
The metadata are in a file,
[leish_meta_data.txt](meta/leish_meta_data.txt). This is a
tab-delimited file. The first column is the sample name and the other 
columns give the "treatments". In this case, the treatment is stage
(with three levels).
...
metadata dataframe is "char" and it has been
converted into a factor type.
...
Note that `stage` is the name of the column in the metadata
dataframe and `procyclic` and `metacyclic` are the names of the levels in the
`stage` column. By putting them in the order `procyclic` , `metacyclic` we
are saying the fold change will be procyclic / metacyclic.  This means:
...
This will give us the log~2~ fold change, the  *p*-value and the 
adjusted  *p*-value for the comparison between procyclic and metacyclic
stage for each gene
...
Now go to [Add gene information](#leishmania-3).
...
The genes expressed in only cell type are those with zeros in
all the cells of the other type. We can find these by summing the
expression values for each gene across the cells of one type and
filtering for those that are zero.

To do row wise aggregates such as the sum across rows we can use 
the `rowwise()` function. `c_across()` allows us
...
🎬 Find the genes expressed only in progenitor cells (*i.e.*, those 
...
```
We already know from last week's work that there are no genes that are zero 
across all the cells (both types). If we did not know that, we would need to add
...
`hspc_prog` is a dataframe so we will use the ensembl gene ids to name the rows
and remove the gene ids from the dataframe.
...
Like **`DESeq2`**, **`scran`** needs metadata to define which
columns were in which group. Instead of having this is a file, we
...
The number of times each cell type is repeated is the number of cells 
of that type. Do check that the length of the `cell_type` vector is
the same as the number of columns in the `hspc_prog` dataframe.
...
Differential expression is carried out with the `findMarkers()` function. It takes two arguments. The first argument is the dataframe containing the data and the
second argument is the vector indicating which columns are in which cell
type. Make sure that the order of the cell types in the vector is appropriate
for the order of the columns in the dataframe.
...
-   The dataframe `results_hspc_prog$hspc` is log hspc - log prog (*i.e.*,
    HSPC/Prog). This means:
...
We only need one of these results dataframes and we will use the `prog` one. 
It does not matter which one you use but you do need keep the direction 
of the foldchange in mind when interpreting the results.

Notice that the dataframes are ordered by significance rather than the original 
gene order. 

It is useful to have the normalised counts and the statistical results in 
one dataframe to which we will add the gene information from Ensembl.
Having all the information together will make it easier to interpret the 
results and select genes of interest. We will need to extract the results from 
the list object, add the gene ids and then join with the normalised counts.

🎬 Extract the results dataframe from the list object and add the 
gene ids as a column:
...
Now go to [Add gene information](#stem-cells-3).
...
number of databases and [BioMart](https://www.ensembl.org/info/data/biomart/index.html)
...
returned only has 279 rows - one of the ids does not have information.
