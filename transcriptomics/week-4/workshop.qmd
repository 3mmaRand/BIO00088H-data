...
The warning "Warning: some variables in design formula are characters,
converting to factors" just means that the variable type of stage in the metadata dataframe is "char" and it has been
converted into a factor type.
...
gene order. We only need one of these results dataframes. We will use the `prog` 
one.
...
results and select genes of interest. We will need to extract the results from the list object, add the gene ids and then join with the normalised counts.
...
```{r}
# merge the results with the normalised counts
hspc_prog_results <- hspc_prog |>
  left_join(hspc_prog_results, by = "ensembl_gene_id")
