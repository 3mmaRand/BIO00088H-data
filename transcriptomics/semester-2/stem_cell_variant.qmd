...
[results/hspc_prog_results.csv](results/hspc_prog_results.csv) file. This works 
because the same 280 genes are in the three raw data files and this results 
file.
...
columns. The first column gives the cell_id (e.g., `HSPC_001`) the next 12 
columns
...
## Add the two types of categorisation
...
Refer to the 
[workshop for more explanation on PCA](../week-5/workshop.html#stem-cells-2).
...
🎬 Add the expression of `Emb` to `pca_labelled` data. I've also renamed the
   expression column from the filtered `cell_expr_long` so it is `Emb_expr` in
   the `pca_labelled` data.
