...
🎬 Import [secretome_lthsc.csv](data-raw/secretome_lthsc.csv),
[secretome_hspc.csv](data-raw/secretome_hspc.csv) and
...
# 🐭 import the lthsc, hspc and prog data
...
This works because the same genes are in the three raw data files and
this results file.
...
Note that this data frame will have: \[280 genes \* 1654 cells =
463120\] rows.
...
I have chosen the gene `Emilin2` to plot the expression in each cell
type. I prefer the use of violin plots and points because multimodal
distributions are more obvious than in boxplots.

🎬 Plot the expression of `Emilin2` in each cell type for the 3-type
categorisation.

```{r}
# 🐭 plot the expression of Emilin2 in each cell type
cell_expr_long |> 
...
🎬 Add the expression of `Emilin2` to `pca_labelled` data. I've also
renamed the expression column from the filtered `cell_expr_long` so it
is `Emilin2_expr` in the `pca_labelled` data.

```{r}
pca_labelled <- pca_labelled |> 
  left_join(cell_expr_long |> 
              filter(external_gene_name == "Emilin2") |> 
              select(cell_id, "Emilin2_expr" = expression), 
            by = "cell_id")
```
...
🎬 Plot PC2 against PC1 and colour by expression of Emilin2 and shape by
cell_type_3 type:
