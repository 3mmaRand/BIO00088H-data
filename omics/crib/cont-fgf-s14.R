...
# 🐸 import the s14 data
s14 <- read_csv("omics/week-3/data-raw/xlaevis_counts_s14.csv")
...
# Write `s14_summary_samp` to a file called "s14_summary_samp.csv":
write_csv(s14_summary_samp, 
          file = "omics/week-3/data-processed/s14_summary_samp.csv")
...
# Write `s14_summary_gene` to a file called "s14_summary_gene.csv":
write_csv(s14_summary_gene, 
          file = "omics/week-3/data-processed/s14_summary_gene.csv")
...
# filter the `s14_summary_gene` dataframe to obtain a list of
# `xenbase_gene_id` we can use to filter `s14`.
...
s14_summary_gene_filtered <- s14_summary_gene |> 
  filter(total > 20) |> 
  filter(n_zero < 4)
...
write_csv(s14_summary_gene_filtered, 
          file = "omics/week-3/data-processed/s14_summary_gene_filtered.csv")


# Use the list of `xenbase_gene_id` in the filtered summary to filter
# the original dataset:
s14_filtered <- s14 |> 
  filter(xenbase_gene_id %in%  s14_summary_gene_filtered$xenbase_gene_id)

# Write the filtered original to file:
write_csv(s14_filtered, 
          file = "omics/week-3/data-processed/s14_filtered.csv")
...
# import the filtered s14
s14_filtered <- read_csv("omics/week-3/data-processed/s14_filtered.csv")
...
# Write to file 
write_csv(s14_fgf_only, "omics/week-4/results/s14_fgf_only.csv")
...
# We only need the s14
meta_s14 <- meta |>
  dplyr::filter(stage == "stage_14")
...
names(s14_filtered[2:7]) == row.names(meta_s14)
...
s14_count_mat <- s14_filtered |>
  select(-xenbase_gene_id) |>
  as.matrix()
# add the genes as row names
row.names(s14_count_mat) <- s14_filtered$xenbase_gene_id

View(s14_count_mat)
knitr::kable(head(s14_count_mat))


# creates the DESeqDataSet (dds) object
dds <- DESeqDataSetFromMatrix(s14_count_mat,
                              colData = meta_s14,
...
counts(dds) |> View()
# This should be what is in s14_count_mat
...
sizeFactors(dds)
# S14_C_5   S14_C_6   S14_C_A   S14_F_5   S14_F_6   S14_F_A 
...
plot(colSums(s14_count_mat), sizeFactors(dds))
...
data.frame(normalised_counts,
  ...
  write_csv(file = "omics/week-4/results/s14_normalised_counts.csv")
...
data.frame(results_fgf,
  ...
  write_csv(file = "omics/week-4/results/s14_results.csv")
...
s14_count_norm <- read_csv("omics/week-4/results/s14_normalised_counts.csv")

s14_count_norm

# DE results
s14_results <- read_csv("omics/week-4/results/s14_results.csv")
s14_results


# merge the results with the normalised counts
s14_results <- s14_count_norm |>
  left_join(s14_results, by = "xenbase_gene_id")
...
# log2 transform the counts in s14_count_norm
# add a tiny amount to avoid log(0)
s14_results <- s14_results |>
  mutate(across(starts_with("s14"), 
                \(x) log2(x + 0.001),
                .names = "log2_{.col}"))
...
s14_results_sig0.01 <- s14_results |> 
  filter(padj <= 0.01)
# 59 genes

# write to csv file
write_csv(s14_results_sig0.01, 
          file = "omics/week-5/results/s14_results_sig0.01.csv")

s14_results_sig0.05 <- s14_results |> 
  filter(padj <= 0.05)

# 117 genes

# write to csv file
write_csv(s14_results_sig0.05, 
          file = "omics/week-5/results/s14_results_sig0.05.csv")
...
s14_log2_trans <- s14_results |> 
  select(starts_with("log2_")) |>
  t() |> 
  data.frame()

colnames(s14_log2_trans) <- s14_results$xenbase_gene_id

# just for indep study before
# s14_log2_trans$sample <- row.names(s14_log2_trans)
# a <- s14_log2_trans |> ggplot(aes(x = `XB-GENE-1000007`, 
#                              y = `XB-GENE-1000023`)) +
#   geom_point() +
...
# 
# 
# b <- s14_log2_trans |> ggplot(aes(x = `XB-GENE-1000062`, 
#                                   y = `XB-GENE-1000072`)) +
#   geom_point() +
#   geom_text(aes(label = sample), 
#             vjust = -1, size = 3) +
...
#   theme_classic()
# 
# c <- s14_log2_trans |> ggplot(aes(x = `XB-GENE-1000113`, 
#                                   y = `XB-GENE-1000132`)) +
#   geom_point() +
#   geom_text(aes(label = sample), 
#             vjust = -1, size = 3) +
...
#   theme_classic()
# 
# d <- s14_log2_trans |> ggplot(aes(x = `XB-GENE-1000149`, 
#                                   y = `XB-GENE-1000251`)) +
#   geom_point() +
#   geom_text(aes(label = sample), 
#             vjust = -1, size = 3) +
...
# remove log2 from  row.names(s14_log2_trans)
# to labelthe pca results
sample_id <- row.names(s14_log2_trans) |> str_remove("log2_")
...
pca <- pca_labelled |> ggplot(aes(x = PC1, y = PC2, 
                                  colour = sibling_rep,
                                  shape = treatment)) +
  geom_point(size = 3) +
...
ggsave("omics/week-5/figures/frog-s14-pca.png",
       plot = pca,
       height = 3, 
       width = 4,
       units = "in",
...
mat <- s14_results_sig0.01 |> 
  select(starts_with("log2_")) |>
  as.matrix()

rownames(mat) <- s14_results_sig0.01$xenbase_gene_symbol
...
vol <- s14_results |> 
  ...
  scale_colour_manual(values = c("gray", 
                                 ...
                                 "gray30",
                                 "deeppink")) +
  geom_text_repel(data = subset(s14_results, 
                                bigfc & sig),
...
ggsave("omics/week-5/figures/frog-s14-volcano.png",
       plot = vol,
       height = 4.5, 
       width = 4.5,
       units = "in",
