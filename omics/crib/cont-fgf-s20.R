...
# 🐸 import the s20 data
s20 <- read_csv("omics/week-3/data-raw/xlaevis_counts_s20.csv")
...
write_csv(s20_summary_samp, 
          file = "omics/week-3/data-processed/s20_summary_samp.csv")
...
write_csv(s20_summary_gene, 
          file = "omics/week-3/data-processed/s20_summary_gene.csv")
...
write_csv(s20_summary_gene_filtered, 
          file = "omics/week-3/data-processed/s20_summary_gene_filtered.csv")
...
write_csv(s20_filtered, 
          file = "omics/week-3/data-processed/s20_filtered.csv")
...
# import the filtered s20
s20_filtered <- read_csv("omics/week-3/data-processed/s20_filtered.csv")
...
# Write to file 
write_csv(s20_fgf_only, "omics/week-4/results/s20_fgf_only.csv")
...
# We only need the s20
meta_s20 <- meta |>
  dplyr::filter(stage == "stage_20")
...
names(s20_filtered[2:7]) == row.names(meta_s20)
...
s20_count_mat <- s20_filtered |>
  select(-xenbase_gene_id) |>
  as.matrix()
# add the genes as row names
row.names(s20_count_mat) <- s20_filtered$xenbase_gene_id

View(s20_count_mat)
knitr::kable(head(s20_count_mat))


# creates the DESeqDataSet (dds) object
dds <- DESeqDataSetFromMatrix(s20_count_mat,
                              colData = meta_s20,
...
counts(dds) |> View()
# This should be what is in s20_count_mat
...
sizeFactors(dds)
# S20_C_5   S20_C_6   S20_C_A   S20_F_5   S20_F_6   S20_F_A 
...
plot(colSums(s20_count_mat), sizeFactors(dds))
...
data.frame(normalised_counts,
  ...
  write_csv(file = "omics/week-4/results/s20_normalised_counts.csv")
...
data.frame(results_fgf,
  ...
  write_csv(file = "omics/week-4/results/s20_results.csv")
...
s20_count_norm <- read_csv("omics/week-4/results/s20_normalised_counts.csv")

s20_count_norm

# DE results
s20_results <- read_csv("omics/week-4/results/s20_results.csv")
s20_results


# merge the results with the normalised counts
s20_results <- s20_count_norm |>
  left_join(s20_results, by = "xenbase_gene_id")
...
# log2 transform the counts in s20_count_norm
# add a tiny amount to avoid log(0)
s20_results <- s20_results |>
  mutate(across(starts_with("s20"), 
                \(x) log2(x + 0.001),
                .names = "log2_{.col}"))
...
s20_results_sig0.01 <- s20_results |> 
  filter(padj <= 0.01)
# 59 genes

# write to csv file
write_csv(s20_results_sig0.01, 
          file = "omics/week-5/results/s20_results_sig0.01.csv")

s20_results_sig0.05 <- s20_results |> 
  filter(padj <= 0.05)

# 117 genes

# write to csv file
write_csv(s20_results_sig0.05, 
          file = "omics/week-5/results/s20_results_sig0.05.csv")
...
s20_log2_trans <- s20_results |> 
  select(starts_with("log2_")) |>
  t() |> 
  data.frame()

colnames(s20_log2_trans) <- s20_results$xenbase_gene_id

# just for indep study before
# s20_log2_trans$sample <- row.names(s20_log2_trans)
# a <- s20_log2_trans |> ggplot(aes(x = `XB-GENE-1000007`, 
#                              y = `XB-GENE-1000023`)) +
#   geom_point() +
...
# 
# 
# b <- s20_log2_trans |> ggplot(aes(x = `XB-GENE-1000062`, 
#                                   y = `XB-GENE-1000072`)) +
#   geom_point() +
#   geom_text(aes(label = sample), 
#             vjust = -1, size = 3) +
...
#   theme_classic()
# 
# c <- s20_log2_trans |> ggplot(aes(x = `XB-GENE-1000113`, 
#                                   y = `XB-GENE-1000132`)) +
#   geom_point() +
#   geom_text(aes(label = sample), 
#             vjust = -1, size = 3) +
...
#   theme_classic()
# 
# d <- s20_log2_trans |> ggplot(aes(x = `XB-GENE-1000149`, 
#                                   y = `XB-GENE-1000251`)) +
#   geom_point() +
#   geom_text(aes(label = sample), 
#             vjust = -1, size = 3) +
...
# remove log2 from  row.names(s20_log2_trans)
# to labelthe pca results
sample_id <- row.names(s20_log2_trans) |> str_remove("log2_")
...
ggsave("omics/week-5/figures/frog-s20-pca.png",
       plot = pca,
       height = 3, 
       width = 4,
       units = "in",
...
mat <- s20_results_sig0.01 |> 
  select(starts_with("log2_")) |>
  as.matrix()

rownames(mat) <- s20_results_sig0.01$xenbase_gene_symbol
...
vol <- s20_results |> 
  ...
  scale_colour_manual(values = c("gray", 
                                 ...
                                 "gray30",
                                 "deeppink")) +
  geom_text_repel(data = subset(s20_results, 
                                bigfc & sig),
...
ggsave("omics/week-5/figures/frog-s20-volcano.png",
       plot = vol,
       height = 4.5, 
       width = 4.5,
       units = "in",
